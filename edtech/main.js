...
// ===========================
// MATERIALS - WEEK COLLAPSIBLE
// ===========================
const weekTitles = document.querySelectorAll('.week-title');

weekTitles.forEach(title => {
    title.addEventListener('click', () => {
        const weekNum = title.getAttribute('data-week');
        const weekMaterials = document.querySelector(`.week-materials[data-week="${weekNum}"]`);

        if (weekMaterials) {
            // Toggle display
            if (weekMaterials.style.display === 'none') {
                weekMaterials.style.display = 'flex';
                title.classList.remove('collapsed');
            } else {
                weekMaterials.style.display = 'none';
...
if (materialsSearch) {
    materialsSearch.addEventListener('input', (e) => {
        ...
        materialCards.forEach(card => {
            const title = card.querySelector('.material-info h5').textContent.toLowerCase();
...
submitBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
        e.preventDefault();
        const assignmentTitle = btn.closest('.assignment-card').querySelector('.assignment-title h4').textContent;
        alert(`Submitting: ${assignmentTitle}\n\nThis will open the submission form.`);
    });
});

// View Feedback Button
const feedbackBtns = document.querySelectorAll('.btn-view-feedback');
feedbackBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
        e.preventDefault();
        const assignmentTitle = btn.closest('.assignment-card').querySelector('.assignment-title h4').textContent;
        const feedback = `Feedback for: ${assignmentTitle}\n\nGreat work! Your HTML structure is well-organized. Consider using more semantic HTML5 tags. Good overall effort!`;
        alert(feedback);
    });
});

// ===========================
// SCHEDULE - JOIN CLASS BUTTON
// ===========================
const joinClassBtns = document.querySelectorAll('.btn-join');
joinClassBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
        e.preventDefault();
        const classTitle = btn.closest('.schedule-item').querySelector('.schedule-details h4').textContent;
        alert(`Joining: ${classTitle}\n\nOpening video conference...`);
    });
});

// ===========================
// MATERIALS - DOWNLOAD/PLAY BUTTONS
// ===========================
const downloadBtns = document.querySelectorAll('.btn-download');
downloadBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
        e.preventDefault();
        const materialName = btn.closest('.material-card').querySelector('.material-info h5').textContent;
...
// YOUTUBE VIDEO LINKS
// ===========================
const videoCardLinks = document.querySelectorAll('.material-card.video-card-link');

videoCardLinks.forEach(card => {
    ...
    card.addEventListener('mouseenter', () => {
        card.style.backgroundColor = 'rgba(102, 126, 234, 0.1)';
    });
...
// ===========================
// DISCUSSIONS - VIEW THREAD
// ===========================
const viewThreadBtns = document.querySelectorAll('.btn-view-thread');
viewThreadBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
        e.preventDefault();
        const threadTitle = btn.closest('.discussion-thread').querySelector('.discussion-header h4').textContent;
        alert(`Opening discussion: "${threadTitle}"\n\nThis will open the full discussion thread where you can read and reply to posts.`);
    });
});

// ===========================
// DISCUSSIONS - START NEW DISCUSSION
// ===========================
const newDiscussionBtn = document.querySelector('.btn-new-discussion');
if (newDiscussionBtn) {
    newDiscussionBtn.addEventListener('click', (e) => {
        e.preventDefault();
        const subject = prompt('Enter discussion subject:');
        if (subject) {
            alert(`Creating new discussion: "${subject}"\n\nThis will open the discussion editor.`);
        }
    });
}

// ===========================
// MAKE TAB BUTTONS PREVENT DEFAULT
// ===========================
classTabBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
        e.preventDefault();
...
// ===========================
// MAKE WEEK TITLES PREVENT DEFAULT
// ===========================
weekTitles.forEach(title => {
...
videoCards.forEach(card => {
    card.addEventListener('click', () => {
        ...
        if (videoId) {
            const video = {
                id: videoId,
                title: card.querySelector('h4') ? card.querySelector('h4').textContent : 'Video',
                channel: card.querySelector('.course-category') ? card.querySelector('.course-category').textContent : 'YouTube',
